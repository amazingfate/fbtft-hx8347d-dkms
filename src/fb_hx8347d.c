...
static int set_var(struct fbtft_par *par)
{
	...
	case 0:
		write_reg(par, 0x16, MEM_V | MEM_X | MEM_Y | (par->bgr << MEM_BGR));
		break;
	case 270:
		write_reg(par, 0x16, MEM_X | par->bgr << MEM_BGR);
		break;
	case 180:
		write_reg(par, 0x16, MEM_V | (par->bgr << MEM_BGR));
		break;
	case 90:
		write_reg(par, 0x16, MEM_Y | (par->bgr << MEM_BGR));
		break;
	}
